...
turf/Terrain
	...
	Grass
		var/mob/Body
		var/bur_info = "Nothing written."
		Click()
			if(!usr.client) return
			if(get_dist(usr,src)>1) return
			if(!Body) return

			var/choice = input(usr,"Choose an option","Grave") in list("Examine", "Dig", "Cancel")
			if(choice == "Examine")
				usr<<output("<font size = -3>The grave reads: [bur_info].","outputic.output")
				usr<<output("<font size = -3>The grave reads: [bur_info].","outputall.output")
			if(choice == "Dig")
...
obj
	...
	Konoha
		...
		stairs
			icon='Turf.dmi'
			icon_state= "stairs123"
		stairsL
			icon='Turf.dmi'
			icon_state= "stairs1234"
		stairsR
			icon='Turf.dmi'
			icon_state= "stairs12345"
turf
	Suna
		SunaBuilding
			icon='Sand.dmi'
